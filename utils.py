...
def iql_normalize(dataset):
    """
    normalized reward <- reward /(max_return-min_return)* 1000.0
...
def sampleqdata(data:collections.defaultdict, batch_size: int) -> collections.defaultdict:
    data_ = collections.defaultdict(list)
    size=data["mcreturn"].shape[0]
    indices = sample_n_k(size, batch_size)
        
    data_["observations"]= data["observations"][indices]
    data_["actions"]= data["actions"][indices]
    data_["mcreturn"]= data["mcreturn"][indices]
    data_["mcreturn_ori"]= data["mcreturn_ori"][indices]
    return data_
    
    
def denormalize_value(value:np.ndarray,Qmax:float,Qmin:float)-> np.ndarray:
    ## [0, 1]
    normed = (value+1)/2
    ##[vmin,vmax]
    normed = normed*(Qmax - Qmin)+Qmin
    return normed
